Supermario Core version 0.13.2 is now available from:

  <https://download.supermario.org/supermario-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/supermario-project/supermario/issues>
...
updates will be released anymore. Without security updates, using a supermario
...
ECDSA signatures inside Supermario transactions now use validation using
...
Previous versions of Supermario Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Supermario Core was to raise the default minimum
relay fee.

Supermario Core 0.13.2 will have a strict maximum size on the mempool. The
...
Supermario Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running supermariod without having to do any manual configuration.
...
Supermario Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Supermario Core will relay transactions with
...
entirely in Supermario Core 0.13, and it is at this time undecided whether the
...
Supermario Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Supermario Core automatically creates a hidden service to listen on, without
manual configuration. Supermario Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Supermario Core is listening, and
...
Supermariod can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Supermario
Core to use floating fees. By default, Supermario Core will use floating
fees.
...
At all times, Supermario Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SPM.
Furthermore, Supermario Core will never create transactions paying less than
...
`supermario.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29822 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29822 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /supermariorpc>
        ProxyPass http://127.0.0.1:9822/
        ProxyPassReverse http://127.0.0.1:9822/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass supermariod rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< supermariorpc:<password>
    </Location>
...
Restarting supermariod will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in supermario.conf
...
supermario-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/supermario-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Supermario Core code base has
...
- `supermario-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `supermario-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Supermario scripting system that in combination with
...
Supermario transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Supermario Core.
...
The two phases are distinct in the Supermario-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Supermario wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Supermario.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Supermario block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of supermarios they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of supermarios being spent is exactly
...
- **Increased security for multisig:** Supermario addresses (both P2PKH addresses
  ...
  believed to be provided by Supermario's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Supermario's current ledger (called the UTXO set),
  ...
  note that Supermario Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Supermario users to individually opt-in to almost any change in the Supermario
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Supermario Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Supermario
client to Supermario Core 0.13.2 to signal for segwit support. When a
...
Since Supermario Core 0.10.0, nodes have defaulted to only relaying and
...
- `supermario-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `supermario-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
