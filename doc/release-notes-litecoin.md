Supermario Core version *0.15.1* is now available from:

  <https://download.supermario.org/supermario-0.15.1/>
...
A number of changes to the way Supermario Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mirs per kB (0.00001 SPM). This is to prepare for dropping the minimum transaction fee to 0.1 mirs per kB (0.0001 SPM) in the next release.
