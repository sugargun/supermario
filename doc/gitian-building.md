...
*Setup instructions for a Gitian build of Supermario Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Supermario
...
- [Building Supermario Core](#building-supermario-core)
...
Clone the git repositories for supermario and Gitian.
...
in the supermario repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Supermario Core
----------------

To build Supermario Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the supermario repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/supermario/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/supermario-project/supermario
...
For example:
```bash
URL=https://github.com/thrasher-/supermario.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit supermario=${COMMIT} --url supermario=${URL} ../supermario/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit supermario=${COMMIT} --url supermario=${URL} ../supermario/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit supermario=${COMMIT} --url supermario=${URL} ../supermario/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the supermario git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../supermario/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/supermario
SIGPATH=/some/root/path/supermario-detached-sigs

./bin/gbuild --url supermario=${BTCPATH},signature=${SIGPATH} ../supermario/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/supermario-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/supermario-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/supermario-osx-build.assert
```
...
[supermario-project/gitian.sigs.mir](https://github.com/supermario-project/gitian.sigs.mir/) repository, or if that's not possible create a pull
